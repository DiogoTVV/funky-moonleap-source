...
class Timings
{
	...
	public static var segmentsHit:Float = 0;
	...
	{
		notesHit++;
		segmentsHit += (judgement / 100);
		
		//trace('Notes Hit:    $notesHit');
		//trace('Segments Hit: $segmentsHit');
		
		trueAccuracy = FlxMath.remapToRange(segmentsHit, 0, notesHit, 0, 100);
	...
	// just silly details
	public static function formatAccuracy(rawAccuracy:Float):String
	{
		rawAccuracy = Math.floor(rawAccuracy * 100) / 100;
		
		var formatAcc:String = Std.string(rawAccuracy);
		var accArray:Array<String> = formatAcc.split('.');
		
		if(!formatAcc.contains('.'))
			formatAcc += '.00';
		else
		{
			if(accArray[1].length < 2)
				formatAcc += '0';
		}
		//if(accArray[0].length < 2)
		//	formatAcc = '0${formatAcc}';
