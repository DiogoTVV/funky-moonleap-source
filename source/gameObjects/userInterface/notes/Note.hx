...
class Note extends FNFSprite
{
	...
	public function new(strumTime:Float, noteData:Int, noteType:String, noteAlt:Float, ?prevNote:Note, ?sustainNote:Bool = false)
	{
		super(x, y);
		...
		// used for sustain notes
		noteCrochet = Conductor.stepCrochet;
		for (event in Conductor.bpmChangeMap)
		{
			if (strumTime >= event.songTime)
	...
	**/
	public static function returnDefaultNote(assetModifier, strumTime, noteData, noteType, noteAlt, ?isSustainNote:Bool = false, ?prevNote:Note = null):Note
	{
		...
		{
			case 'pixel':
				var mult:Int = ((newNote.noteType == "Night Note") ? 1 : 0);
				if(!isSustainNote)
				{
					mult *= 4;
					newNote.loadGraphic(Paths.image('noteskins/moonleap/pixel/pixelNotes'),true, 17, 17);
					newNote.animation.add('purpleScroll',[0 + mult]);
					newNote.animation.add('blueScroll',  [1 + mult]);
					newNote.animation.add('greenScroll', [2 + mult]);
					newNote.animation.add('redScroll', 	 [3 + mult]);
				}
				else
				{
					newNote.loadGraphic(Paths.image('noteskins/moonleap/pixel/holdPixelNotes'),true, 10, 13);
					
					for(w in 0...4)
					{
						var daColor:String = StrumNote.getColorFromNumber(w);
						newNote.animation.add('${daColor}hold',   [0 + mult]);
						newNote.animation.add('${daColor}holdend',[2 + mult]);
					}
					
					newNote.noteVisualOffset = -16;
				}
				...
				if (isSustainNote)
				{
					newNote.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('arrowEnds', assetModifier, SaveData.trueSettings.get("Note Skin"),
				...
				{
					newNote.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('arrows-pixels', assetModifier, SaveData.trueSettings.get("Note Skin"),
						'noteskins/notes')),
						true, 17, 17);
			...
			default: // base game arrows for no reason whatsoever
				...
				{
					default:
						newNote.noteVisualOffset = -3;
						newNote.frames = Paths.getSparrowAtlas('noteskins/moonleap/base/NOTE_ASSETS');
				}
				
				// i fuckin hate this "sort by color" system >:[
				newNote.animation.addByPrefix('purpleScroll', 'left note');
				newNote.animation.addByPrefix('blueScroll',   'down note');
				newNote.animation.addByPrefix('greenScroll',  'up note');
				newNote.animation.addByPrefix('redScroll', 	  'right note');
				
				newNote.animation.addByPrefix('purplehold',	  'left hold0');
				newNote.animation.addByPrefix('bluehold', 	  'down hold0');
				newNote.animation.addByPrefix('greenhold',	  'up hold0');
				newNote.animation.addByPrefix('redhold',  	  'right hold0');
				
				newNote.animation.addByPrefix('purpleholdend','left hold end');
				newNote.animation.addByPrefix('blueholdend',  'down hold end');
				newNote.animation.addByPrefix('greenholdend', 'up hold end');
				newNote.animation.addByPrefix('redholdend',   'right hold end');
				
				newNote.scale.set(0.7,0.7);
				newNote.updateHitbox();
				...
				{
					default:
						newNote.frames = Paths.getSparrowAtlas(ForeverTools.returnSkinAsset('NOTE_assets', assetModifier, SaveData.trueSettings.get("Note Skin"), 'noteskins/notes'));
				}
		...
		//
		if (!isSustainNote)
			newNote.animation.play(StrumNote.getColorFromNumber(noteData) + 'Scroll');
		// trace(prevNote);
		if (isSustainNote && prevNote != null)
		{
			//newNote.noteSpeed = prevNote.noteSpeed;
			newNote.alpha = (SaveData.trueSettings.get('Opaque Holds')) ? 1 : 0.6;
			newNote.animation.play(StrumNote.getColorFromNumber(noteData) + 'holdend');
			newNote.updateHitbox();
			if (prevNote.isSustainNote)
			{
				prevNote.animation.play(StrumNote.getColorFromNumber(prevNote.noteData) + 'hold');
	...
	public static function returnQuantNote(assetModifier, strumTime, noteData, noteType, noteAlt, ?isSustainNote:Bool = false, ?prevNote:Note = null):Note
	{
		...
		if (newNote.noteQuant == -1)
		{
			...
			for (event in Conductor.bpmChangeMap)
			{
				...
				{
					curBPM = event.bpm;
					newTime = strumTime - event.songTime;
				}
			}
			...
			for (quant in 0...quantArray.length)
			{
				...
				if ((newTime #if !neko + SaveData.trueSettings['Offset'] #end + smallestDeviation) % quantTime < smallestDeviation * 2)
				{
		...
		switch (assetModifier)
		{
			default:
				...
				if (!isSustainNote)
				{
					...
					newNote.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('NOTE_quants', assetModifier, SaveData.trueSettings.get("Note Skin"),
				...
				{
					// quant holds
					newNote.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('HOLD_quants', assetModifier, SaveData.trueSettings.get("Note Skin"),
						'noteskins/notes', 'quant')),
		...
		//
		if (!isSustainNote)
			newNote.animation.play(StrumNote.getArrowFromNumber(noteData) + 'Scroll');
