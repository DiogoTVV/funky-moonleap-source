...
class StrumNote extends FlxSprite
{
	...
	public var setAlpha:Float = (SaveData.trueSettings.get('Opaque Arrows')) ? 1 : 0.8;
...
class Strumline extends FlxTypedGroup<FlxBasic>
{
	...
	public function new(x:Float = 0, ?character:Character, ?displayJudgements:Bool = true, ?autoplay:Bool = true,
			?noteSplashes:Bool = false, ?keyAmount:Int = 4, ?downscroll:Bool = false, ?parent:Strumline)
	{
		...
		{
			var staticArrow:StrumNote = ForeverAssets.generateUIArrows(-25 + x, 25 + (downscroll ? FlxG.height - 200 : 0), i, PlayState.assetModifier);
			staticArrow.ID = i;
		...
		switch(SaveData.trueSettings.get("Clip Style").toLowerCase())
		{
			case "stepmania":
				add(holdsGroup);
				add(receptors);
