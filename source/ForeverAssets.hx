...
class ForeverAssets
{
	...
	public static function generateRating(asset:String, perfectSick:Bool, assetModifier:String = 'base'):FlxSprite
	{
		var ratingGraphic = Paths.image('UI/default/$assetModifier/moon-judgements');
		
		var rating = new FlxSprite().loadGraphic(ratingGraphic, true, Math.floor(ratingGraphic.width / 6), Math.floor(ratingGraphic.height));
		
		rating.alpha = 1;
		rating.acceleration.y = 550;
		rating.velocity.y = -FlxG.random.int(140, 175);
		//rating.velocity.x = -FlxG.random.int(0, 10);
		rating.animation.add('saulgoodman', [Std.int(Timings.judgementsMap.get(asset)[0]) + (perfectSick ? 0 : 1)], 24, false);
		rating.animation.play('saulgoodman'); // get it because its a JUDGEment // that doesnt even make sense he's a lawyer // shut up
			...
			noteData:Int):NoteSplash
	{
		...
		{
			case 'pixel':
				tempSplash.loadGraphic(Paths.image('UI/default/pixel/pixelSplash'), true, 51, 51);
				
				tempSplash.animation.add('anim1', [0,1,2,3,3,4,4,5,5,6,6], 24, false);
				tempSplash.addOffset('anim1', -100, -75);
				tempSplash.setGraphicSize(Std.int(tempSplash.width * PlayState.daPixelZoom));
				tempSplash.updateHitbox();
				
			default:
				var stringSect:String = StrumNote.getArrowFromNumber(noteData).toLowerCase();
				
				tempSplash.frames = Paths.getSparrowAtlas('UI/default/base/SPLASHES_ASSETS');
				//var strI:String = Std.string(i);
				
				tempSplash.animation.addByPrefix('anim1', stringSect + 'Splash', 24, false);
				tempSplash.animation.play('anim1');
				tempSplash.addOffset('anim1', 28, 24);
				
			case 'pixel-old':
				tempSplash.loadGraphic(
					Paths.image(ForeverTools.returnSkinAsset('splash-pixel', assetModifier, changeableSkin, baseLibrary)), true, 34, 34);
	...
	public static function generateUIArrows(x:Float, y:Float, ?staticArrowType:Int = 0, assetModifier:String):StrumNote
	{
		var newStaticArrow:StrumNote = new StrumNote(x, y, staticArrowType);
		switch (assetModifier)
		{
			case 'pixel-old':
				...
				newStaticArrow.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('$framesArgument', assetModifier, SaveData.trueSettings.get("Note Skin"),
			...
			case 'pixel':
				...
				// not even just a cleanliness thing it's just so annoying to tweak if something goes wrong like
				// genuinely more programmers should make their code more modular
				newStaticArrow.loadGraphic(Paths.image('noteskins/moonleap/pixel/pixelStrums'), true, 17, 17);
				newStaticArrow.animation.add('static', [staticArrowType]);
				newStaticArrow.animation.add('pressed', [4 + staticArrowType], 12, false);
				newStaticArrow.animation.add('confirm', [8 + staticArrowType, 12 + staticArrowType], 24, false);
				
				newStaticArrow.setGraphicSize(Std.int(newStaticArrow.width * PlayState.daPixelZoom));
				newStaticArrow.updateHitbox();
				newStaticArrow.antialiasing = false;
				
				for(anim in newStaticArrow.animation.getNameList())
					//newStaticArrow.addOffset(anim, -67, -50);
					newStaticArrow.addOffset(anim, -67, -60);
				
			default:
				var stringSect:String = StrumNote.getArrowFromNumber(staticArrowType);
				
				newStaticArrow.frames = Paths.getSparrowAtlas('noteskins/moonleap/base/STRUM_ASSETS');
				
				newStaticArrow.animation.addByPrefix('static', 'arrow' + stringSect.toUpperCase() + '0');
				newStaticArrow.animation.addByPrefix('pressed','arrow' + stringSect.toUpperCase() + 'pressed', 24, false);
				newStaticArrow.animation.addByPrefix('confirm','arrow' + stringSect.toUpperCase() + 'glow', 24, false);
				
				//newStaticArrow.antialiasing = true;
				newStaticArrow.scale.set(0.65,0.65);
				...
				var midC:Array<Int> = [14, 16];
				switch(stringSect.toLowerCase())
				{
					case 'down' | 'up':
						midC = [16, 14];
				}
				
				newStaticArrow.addOffset('static', 35, 40);
				newStaticArrow.addOffset('pressed', 35, 40);
				newStaticArrow.addOffset('confirm', midC[0] + 35, midC[1] + 40);
			...
			case 'fnf':
				...
				stringSect = StrumNote.getArrowFromNumber(staticArrowType);
				...
				newStaticArrow.frames = Paths.getSparrowAtlas(ForeverTools.returnSkinAsset('$framesArgument', assetModifier,
					SaveData.trueSettings.get("Note Skin"), 'noteskins/notes'));
	...
	**/
	...
	{
		var newNote:Note;
		var changeableSkin:String = SaveData.trueSettings.get("Note Skin");
